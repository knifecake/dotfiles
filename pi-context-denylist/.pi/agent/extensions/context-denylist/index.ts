...
function formatContextFilesForPrompt(contextFiles: ContextFile[]): string {
	if (contextFiles.length === 0) return "";

	let block = "\n\n<project_context>\n\nProject-specific instructions and guidelines:\n\n";
	for (const { path: filePath, content } of contextFiles) {
		block += `<project_instructions path="${filePath}">\n${content}\n</project_instructions>\n\n`;
	}
	return `${block}</project_context>\n`;
}

function replaceProjectContextSection(
	prompt: string,
	contextFiles: ContextFile[],
	filteredContextFiles: ContextFile[],
): string {
	const original = formatContextFilesForPrompt(contextFiles);
	const replacement = formatContextFilesForPrompt(filteredContextFiles);

	// Prefer replacing the exact block generated by Pi. The fallback handles a
	// block that an earlier extension has already modified.
	if (original && prompt.includes(original)) return prompt.replace(original, replacement);
	return prompt.replace(/\n\n<project_context>\n[\s\S]*?<\/project_context>\n?/, replacement);
}

function replaceSkillsSection(prompt: string, skills: Skill[], filteredSkills: Skill[]): string {
...
export default function contextDenylist(pi: ExtensionAPI) {
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		let systemPrompt = replaceProjectContextSection(event.systemPrompt, contextFiles, filteredContextFiles);
		systemPrompt = replaceSkillsSection(systemPrompt, skills, filteredSkills);
