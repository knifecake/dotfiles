...
export default function webSearchExtension(pi: ExtensionAPI) {
	pi.registerTool({
		name: "web_search",
		label: "Web Search",
		description: `Search the web for current information. Uses Brave Search when BRAVE_API_KEY is set, otherwise falls back to Exa MCP. Output is truncated to ${DEFAULT_MAX_LINES} lines or ${formatSize(DEFAULT_MAX_BYTES)} (whichever is hit first).`,
		promptSnippet: "Search the public web for up-to-date information and sources",
		promptGuidelines: [
			"Use this tool for current events, recent documentation, release notes, and facts beyond model cutoff.",
			"Include the current year in time-sensitive queries (for example: 'AI news 2026').",
			"Prefer provider='auto' unless the user explicitly asks for Brave or Exa.",
		],
		parameters: WebSearchSchema,
		async execute(_toolCallId, rawParams, signal) {
			const params = rawParams as WebSearchParams;
			const searchResult = await runSearch(params, signal);

			let output = searchResult.output;
			if (searchResult.fallbackFrom === "brave") {
				output = `[Brave Search unavailable, used Exa fallback]\n\n${output}`;
			}

			const truncated = await maybeTruncateOutput(output);
			const details: WebSearchDetails = {
				query: params.query,
				provider: searchResult.provider,
				resultCount: searchResult.resultCount,
				fallbackFrom: searchResult.fallbackFrom,
